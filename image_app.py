...
class ImageForm(FlaskForm):
    ...
    darken = FloatField('Коэффициент затемнения (0-1)', validators=[
        InputRequired(),
        NumberRange(min=0.0, max=1.0, message='Должно быть между 0 и 1')
    ])
    recaptcha = RecaptchaField()
    submit = SubmitField('Отправить')

def split_and_shift_image(image_path, darken_factor):
    ...
    for i in range(4):
        enhancer = ImageEnhance.Brightness(parts[i])
        parts[i] = enhancer.enhance(darken_factor)
...
def index():
    ...
    if form.validate_on_submit():
        ...
        darken = form.darken.data

        shifted_image = split_and_shift_image(original_image_path, darken)
